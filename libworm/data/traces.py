...
def load_trace(file_name="data/2022-08-02-01.json"):
    ...
    with open(file_name, "r") as file:
        ...
        neuron_trace = np.array(data_dict["trace_original"], dtype=np.double).T
        neuron_z_trace = np.array(data_dict["trace_array"], dtype=np.double)
        timestamps = np.array(data_dict["timestamp_confocal"], dtype=np.double)
