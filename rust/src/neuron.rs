...
impl Network {
    pub fn new(
        ...
        let leak_int: Vec<f64> = leak_g.iter().zip(leak_e.iter()).map(|(g, e)| g * e).collect();
    ...
    pub fn run(&mut self, mut voltage: Vec<f64>, mut gates: Vec<f64>, timestep: f64, runtime: f64) -> (Vec<f64>, Vec<f64>){
    ...
    fn step(&mut self, voltage: Vec<f64>, gates: Vec<f64>, timestep: f64) -> (Vec<f64>, Vec<f64>) {
        ...
        for (i, line) in self.gap_g.iter().enumerate(){
            self.gap_int[i] = line.iter().zip(&self.gap_indices[i]).map(|(g, index)| g * voltage[*index]).sum()
        }

        let v_inf = izip!(&self.syn_co, &self.syn_int, &self.gap_co, &self.gap_int, &self.leak_g, &self.leak_int)
        .map(|(s_co, s_int, g_co, g_int, l_co, l_int)| (l_int + s_int + g_int) / (l_co + s_co + g_co));
        ...
        let delta_voltage = izip!(leak_current, syn_current, gap_current).map(|(l, s, g)| (-l-s-g) * timestep);
        let new_gates: Vec<f64> = izip!(&voltage, &self.gate_beta, &self.gate_adjust).map(|(v, b, a)| Self::direct_s(*v, *b, *a)).collect();
        let new_voltage = izip!(&voltage, delta_voltage, v_inf).map(|(volt, del, inf)| volt + del.clamp(-(volt - inf).abs(), (volt - inf).abs())).collect();
